...
module modibm
  ...
  !< SvdL, 20250214: set this up such that it is later easily extendable to put in types
  ...
  logical, allocatable :: fluid_mask(:,:,:) !< (1 Byte logical field carrying) internal building points.
...
contains
  ...
  subroutine initibm
    ...
    Nair (:)  = 0
    Nairl(:)  = 0
    Nair (k1) = ijtot ! prevent zero division on usused points
    Nairl(k1) = ijtot ! probably not really required.. assumes buildings don't go over kmax (which they shouldn't)
      ...
      do j=2,j1
         do k=1,kmax
            if (zf(k) <= bc_height(i+myidx*imax,j+myidy*jmax)) then  
              fluid_mask(i,j,k) = .true.      ! Set grid cell to obstacle
    ...
    ! !> Count amount of fluid cells per vertical level
    do i=2,i1
      do j=2,j1
        do k=1,kmax
          if (.not.(fluid_mask(i,j,k))) Nairl(k) = Nairl(k) + 1
        end do
      end do
    end do
    ...
    !!  u-positions (with index i) is to the left of grid center (with index i)
    ...
    allocate(iobst (Nobst,3))                                                                     !< this one is not temporary (can be immediately set to correct size)
      ...
      do j=2,j1
        do i=2,i1
          ...
          if (fluid_mask(i,j,k) .neqv. fluid_mask(i-1,j,k)) then  ! signals wall in x-direction
            if ( fluid_mask(i,j,k) ) then               ! wall with normal in minus x-direction
          ...
          if (fluid_mask(i,j,k) .neqv. fluid_mask(i,j-1,k)) then  ! signals wall in y-direction
            if ( fluid_mask(i,j,k) ) then               ! wall with normal in minus y-direction
    ...
    !$acc enter data copyin(fluid_mask, iobst, ixw_p, ixw_m, iyw_p, iyw_m, izw_p, Nair)
  ...
  subroutine exitibm
    ...
    !$acc exit data delete(fluid_mask, iobst, ixw_p, ixw_m, iyw_p, iyw_m, izw_p, Nair)
  ...
  subroutine applyibm
    ...
    ! SvdL, 20250219: keep this statement for now to get exact reproduction of Stephan's version (change later)
    ! actually just remove it, as it won't anyways do what is wanted.........
    ! thlibm = thl0av(1) !assumes inside air has the same temperature as the air outside, at near-surface level << Strange one..
    ! qtibm  = qt0av (1) 
    ...
    do nn = 1,Nobst
      ...
      e12p(i,j,k) = (e12min - e120(i,j,k))*rk3coefi   !< SvdL, 20250219: not completely sure why this is the proper way.
                                                      !< Not better to explicitly set eddy diffusivities zero here (or not allowed for dtmin calculation)  
      do nc=1,nsv
    ...
    !> All corrections consist of 2 parts: cancel the "wrong" diffusion imposed by modsubgrid at fluid points at the wall, and add wall friction
    ...
    !> Set ghost cells for tendencies as wall may be on physical domain such that correction is on halo cell
  ...
  subroutine zerowallvelocity !<- MK: Force velocity at the immersed boundaries to 0 for a better interaction with the poissonsolver
    ...
    do nn = 1,Nobst
      ...
      up(i+1,j,k) = -um(i+1,j,k) * rk3coefi
      vp(i,j+1,k) = -vm(i,j+1,k) * rk3coefi
      wp(i,j,k+1) = -wm(i,j,k+1) * rk3coefi
